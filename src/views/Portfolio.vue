    ...
    <div class="px-6 md:px-10 max-w-[1600px] mx-auto mb-12 relative overflow-hidden group/filter">
      ...
      >
        <button v-for="tag in tags" :key="tag" 
          ...
          :class="activeTag === tag ? 'bg-zinc-900 text-white dark:bg-white dark:text-black border-zinc-900 dark:border-white' : 'bg-white dark:bg-zinc-900/50 text-zinc-500 dark:text-zinc-400 border-zinc-200 dark:border-white/10 hover:border-zinc-400 dark:hover:border-white/30 hover:text-zinc-900 dark:hover:text-white'"
        >
          {{ tag }}
        </button>
      </div>
    ...
    <main class="px-6 md:px-10 pb-32 max-w-[1600px] mx-auto columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-3 space-y-3">
      ...
      <template v-else>
        <div v-for="(item, index) in filteredItems" :key="index" 
          ...
          <img :src="getThumbnail(item)" :alt="item.title" class="w-full h-auto block group-hover:brightness-50 transition-all duration-700">
